...
type DashboardAwareQuestionProgressRingProps = {
...
export function DashboardAwareQuestionProgressRing({
  answeredCount,
  totalCount = 100,
  label,
}: DashboardAwareQuestionProgressRingProps) {
  const locale = useLocale();
  const pathname = usePathname();
  const normalizedPathname = pathname.replace(/\/+$/, '');
  const dashboardPathname = `/${locale}/dashboard`;

  if (
    normalizedPathname === dashboardPathname ||
    normalizedPathname.startsWith(`${dashboardPathname}/`)
  ) {
    return null;
  }
  ...
  const progress = Math.min(100, Math.round((remainingCount / total) * 100));
