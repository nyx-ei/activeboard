export const APP_EVENTS = {
  authCallbackSucceeded: 'auth_callback_succeeded',
  accountWelcomeEmailSent: 'account_welcome_email_sent',
  accountWelcomeEmailFailed: 'account_welcome_email_failed',
  passwordChangedEmailSent: 'password_changed_email_sent',
  passwordChangedEmailFailed: 'password_changed_email_failed',
  groupCreated: 'group_created',
  groupJoined: 'group_joined',
  groupInviteSent: 'group_invite_sent',
  groupInviteEmailSent: 'group_invite_email_sent',
  groupInviteEmailFailed: 'group_invite_email_failed',
  groupMemberAddedEmailSent: 'group_member_added_email_sent',
  groupMemberAddedEmailFailed: 'group_member_added_email_failed',
  groupInviteAccepted: 'group_invite_accepted',
  groupInviteDeclined: 'group_invite_declined',
  onTheFlyInviteVerificationPassed: 'on_the_fly_invite_verification_passed',
  onTheFlyInviteVerificationBlocked: 'on_the_fly_invite_verification_blocked',
  onTheFlyInviteAccepted: 'on_the_fly_invite_accepted',
  onTheFlyInviteDeclined: 'on_the_fly_invite_declined',
  groupMemberAdded: 'group_member_added',
  sessionJoinedByCode: 'session_joined_by_code',
  sessionScheduled: 'session_scheduled',
  sessionStarted: 'session_started',
  sessionEnded: 'session_ended',
  questionLaunched: 'question_launched',
  answerSubmitted: 'answer_submitted',
  answerRevealed: 'answer_revealed',
  questionClassified: 'question_classified',
  personalReflectionSaved: 'personal_reflection_saved',
  sessionReminderSent: 'session_reminder_sent',
  sessionReminderFailed: 'session_reminder_failed',
  sessionCalendarInviteSent: 'session_calendar_invite_sent',
  sessionCalendarInviteFailed: 'session_calendar_invite_failed',
  trialWarningEmailSent: 'trial_warning_email_sent',
  trialWarningEmailFailed: 'trial_warning_email_failed',
  userScheduleUpdated: 'user_schedule_updated',
  billingSetupStarted: 'billing_setup_started',
  billingSetupCompleted: 'billing_setup_completed',
  subscriptionCheckoutStarted: 'subscription_checkout_started',
  subscriptionCheckoutCompleted: 'subscription_checkout_completed',
  billingPortalOpened: 'billing_portal_opened',
  pwaInstallPromptShown: 'pwa_install_prompt_shown',
  pwaInstallAccepted: 'pwa_install_accepted',
  pwaLaunchedFromHomeScreen: 'pwa_launched_from_home_screen',
  performanceTraceRecorded: 'performance_trace_recorded',
  stripeWebhookReceived: 'stripe_webhook_received',
  stripeWebhookSynced: 'stripe_webhook_synced',
  stripeWebhookIgnored: 'stripe_webhook_ignored',
  stripeWebhookSyncFailed: 'stripe_webhook_sync_failed',
  stripeInvoicePaymentFailed: 'stripe_invoice_payment_failed',
  leaderPassed: 'leader_passed',
} as const;

export type AppEventName = (typeof APP_EVENTS)[keyof typeof APP_EVENTS];
