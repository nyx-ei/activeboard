...
async function createAuthAndProfileUsers() {
  ...
  for (const user of TEST_USERS) {
    ...
    if (authError || !createdAuthUser?.user?.id) {
      throw (
        authError ?? new Error(`Failed to create auth user for ${user.email}`)
      );
    }
...
async function createSessions(groupMap, userMap) {
  ...
  for (const session of TEST_SESSIONS) {
        ...
        started_at:
          typeof session.startedAtOffsetMinutes === 'number'
            ...
            : typeof session.startedAtOffsetHours === 'number'
              ...
              : null,
        ended_at:
          typeof session.endedAtOffsetHours === 'number'
            ? isoFromOffset({ hours: session.endedAtOffsetHours })
            : null,
      })
    ...
    if (sessionError || !insertedSession) {
      throw (
        sessionError ?? new Error(`Failed to create session ${session.name}`)
      );
    }
    ...
    sessionMap.set(session.key, {
      ...
      shareCode: insertedSession.share_code,
    });
  }
...
async function loadExistingQaMaps() {
  const userEmails = TEST_USERS.map((user) => user.email);
  const groupNames = TEST_GROUPS.map((group) => group.name);
  ...
  for (const user of TEST_USERS) {
    ...
    if (!row) {
      throw new Error(
        `Missing QA user ${user.email}. Run npm run seed:qa-test-data first.`,
      );
    }

    userMap.set(user.key, { ...user, id: row.id });
  }
  ...
  for (const group of TEST_GROUPS) {
    ...
    if (!row) {
      throw new Error(
        `Missing QA group ${group.name}. Run npm run seed:qa-test-data first.`,
      );
    }
...
async function cleanupQaSessions(groupMap) {
  const groupIds = [...groupMap.values()].map((group) => group.id);
  if (groupIds.length === 0) {
    return;
  }
...
async function resetQaSessions() {
  const { userMap, groupMap } = await loadExistingQaMaps();
  await cleanupQaSessions(groupMap);
...
async function seedQuestionsAndAnswers(sessionMap, userMap) {
  ...
  const { error: activeAnswersError } = await supabase
    .schema('public')
    .from('answers')
    .insert([
      {
  ...
  const { data: completedQuestions, error: completedQuestionsError } =
    await supabase
      .schema('public')
      .from('questions')
      .insert(
  ...
  if (completedQuestionsError || !completedQuestions) {
    throw (
      completedQuestionsError ??
      new Error('Failed to create completed questions')
    );
  }

  const questionByOrder = new Map(
    completedQuestions.map((question) => [question.order_index, question]),
  );
...
function printSummary(groupMap, sessionMap) {
  ...
  for (const group of TEST_GROUPS) {
    ...
    console.log(
      `- ${createdGroup.name} (invite code: ${createdGroup.inviteCode})`,
    );
  }
  ...
  for (const session of TEST_SESSIONS) {
    ...
    console.log(
      `- ${createdSession.name} (${createdSession.status}) -> ${createdSession.shareCode}`,
    );
  }
}
